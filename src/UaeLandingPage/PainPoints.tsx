  ...
  return (
    <section className="py-20 lg:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        ...
        >
          <h2 className={`text-4xl sm:text-5xl font-bold text-gray-900 mb-4 ${isRTL ? 'text-right' : ''}`}>
            {language === 'ar' ? (
              <>
                الموارد البشرية في الإمارات معطلة.
                <br />
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            {language === 'ar' 
              ? 'لا مزيد من جداول البيانات، لا مزيد من الرفض، لا مزيد من التوتر.'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {cards.map((c, i) => (
            <motion.div
              ...
              className={`group relative bg-white rounded-2xl p-8 shadow-md hover:shadow-xl transition-all border border-blue-50 ${isRTL ? 'text-right' : ''}`}
            >
              <div className={`w-14 h-14 rounded-xl bg-blue-50 flex items-center justify-center mb-6 group-hover:bg-blue-100 transition-colors ${isRTL ? 'ml-auto' : ''}`}>
